...
{
	if(timerTask) {
		MutexLockGuard autolock(_mutex1);
...
{
	if(timerTask) {
		MutexLockGuard autolock(_mutex2);
		_pendingRemoveList.push_back(timerTask);
...
void TimerManager::handleTask()
{
	...
	{
		MutexLockGuard autolock(_mutex2);
		for(auto premoveTask : _pendingRemoveList) {
			for(auto ptask : _taskList) {
				if(ptask == premoveTask)  {
					_taskList.remove(premoveTask);
					break;
				}
			}
		}
	...
	{
		MutexLockGuard autolock(_mutex1);
		for(auto ptask : _pendingTaskList) {
