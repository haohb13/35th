...
int main(int argc, const char *argv[])
{
	...
	//创建epoll实例
	//int efd = epoll_create(MAXEVENTS); //过时了
	...
	{
		int idx;
		//int nready = epoll_wait(efd, events, MAXEVENTS, 5000);// 5秒钟后超时	
		int nready = epoll_wait(efd, events, MAXEVENTS, -1);// 无限等待
		for(idx = 0; idx != nready; ++idx)
		{
			...
			{//新客户端连接到来
                ...
                //不能写死，不够灵活，下次要改的时候，只能改源码，不符合设计原则中的开闭原则

                //在这里挖一个坑, 交给用户填充
                onConnection();//设计一个函数对象接口, 注册过来即可

            } else
			{//与客户端建立的fd --> 已经建立的链接(旧链接)
				if(events[idx].events & EPOLLIN)
				{
					char recvbuf[1024] = {0};
					...
					{ //链接断开之后的处理
						...
						close(fd);
                        //
                        //printf("server:ip:port --->  client:ip:port close\n");
                        onClose();// void(const TcpConnectionPtr & conn);
						continue;
					}

                    //拿到数据之后，接下里就是要去做业务逻辑的处理
                    //decode
                    //compute
                    //encode
                    
                    onMessage();//设计一个函数对象(回调函数), 以提供给用户进行定制化的操作
